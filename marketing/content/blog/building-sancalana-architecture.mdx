---
title: "Building Sancalana: Our Architecture Decisions"
...
The first version of Sancalana could draw a process map. That was the easy part.

The hard part was making the map stay useful after the data became real: millions of events, messy activity names, long-running cases, rework loops, filters that users expect to feel instant, and source systems that were never designed for process analysis.

So the architecture had to do two things at once. It had to treat process mining as a serious data-engineering problem, and it had to keep the product interactive enough for an operations leader to explore without waiting on a batch job.

These are the decisions that shaped the system.
...
The lesson is that process mining is not just a graph problem. It is storage, ingestion, indexing, modeling, rendering, and product latency all stacked together. The algorithms matter, but the architecture around them decides whether anyone can actually use the output.

That is the engineering problem we are solving at Sancalana: make messy enterprise process data fast enough, stable enough, and clear enough that people can reason from it while the problem is still fresh.

[See the platform in action](/platform) or [talk to our engineering team](/contact).
