...
Small process mining demos are deceptively easy. Load a CSV, discover a process map, click around, declare victory.

Enterprise logs are different. Discovery, conformance checking, bottleneck analysis, and variant frequency all touch large parts of the dataset. At that scale, the product stops being a visualization problem and becomes an infrastructure problem.

Here is how we built Sancalana to handle 10 million events per day without making the user wait on every question.
...
The main lesson is simple: performance is part of the product, not an implementation detail. If exploration feels slow, people stop asking the second and third questions - which are usually where the real insight is.
